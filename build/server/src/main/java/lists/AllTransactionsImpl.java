...
@Service
public class AllTransactionsImpl extends AbsDataQueryImpl {
  @Autowired private EntityManager em;
  @Autowired private GqlToSql gqlToSql;

  public AllTransactions get() {
    List<NativeObj> rows = getNativeResult();
    List<Transaction> result = new ArrayList<>();
    for (NativeObj _r1 : rows) {
      result.add(NativeSqlUtil.get(em, _r1.getRef(0), Transaction.class));
    }
    AllTransactions wrap = new AllTransactions();
    wrap.items = result;
    return wrap;
  }
  ...
  public List<NativeObj> getNativeResult() {
    String sql = "select a._id a0 from _transaction a";
