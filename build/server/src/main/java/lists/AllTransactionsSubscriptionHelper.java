...
public class AllTransactionsSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allTransactionsImpl.getAsJson(field, event.nativeData);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allTransactionsImpl.getNativeResult();
  ...
  private List<NativeObj> createTransactionData(Transaction transaction) {
    ...
    NativeObj row = new NativeObj(transaction.getId());
    data.add(row);
    return data;
  }
